...
2. Authentication > Sign-in method에서 Phone provider를 활성화합니다.
3. Project settings > General에서 Web App을 추가합니다.
4. Web config 값을 `.env`에 입력합니다.
5. 개발 중에는 Authentication > Phone numbers for testing에 테스트 번호를 등록하면 SMS 과금 없이 확인할 수 있습니다.
...
2. 개인정보처리방침 전문을 끝까지 읽습니다.
3. Firebase 휴대폰 번호 인증을 완료합니다.
4. 개인정보 수집 및 이용 동의 후 가입을 완료합니다.
5. 발급된 6자리 토큰을 Discord 봇 명령어에 입력합니다.
6. 봇이 `/api/verify`를 호출하면 서버가 암호화된 사용자 설정을 저장합니다.
